...
def build_dbs():
    ...
    db_path = constants.DB_PATH + constants.DB_FILE_NAME  
    try:
...
def load_data_into_db():
    ...
    """
    # Read the csv file
   # data = pd.read_csv(constants.DATA_DIRECTORY) # uncomment this during training
    data = pd.read_csv(constants.LEAD_SCORING_FILE)  # comment this during training
...
def map_city_tier():
    ...
    df['city_tier'] = df['city_mapped'].map(city_tier_mapping.city_tier_mapping).fillna(3.0)
...
def map_categorical_vars():
    ...
    df['first_platform_c'] = df['first_platform_c'].apply(lambda x: x if x in significant_categorical_level.list_platform else 'Other')

    # Map the insignificant levels of first_utm_medium_c to 'Other'
    df['first_utm_medium_c'] = df['first_utm_medium_c'].apply(lambda x: x if x in significant_categorical_level.list_medium else 'Other')

    # Map the insignificant levels of first_utm_source_c to 'Other'
    df['first_utm_source_c'] = df['first_utm_source_c'].apply(lambda x: x if x in significant_categorical_level.list_source else 'Other')
